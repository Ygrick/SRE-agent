...
| Решение | Обоснование |
|---|---|
| **Codex CLI как ядро SRE-агента** | Codex имеет production-ready shell execution с песочницей, approval modes, обработкой ошибок. Режимы `exec` и MCP server позволяют интегрировать его в автоматизированный pipeline. Поддерживает любые OpenAI-compatible провайдеры. |
| **LLM Gateway как единая точка входа** | Все LLM-запросы (от Codex и других агентов) проходят через Gateway. Это даёт единую точку сбора метрик, маршрутизации и контроля доступа. OpenAI-compatible API (`/v1/chat/completions`) обеспечивает прозрачность для клиентов. |
| **A2A Protocol (Google, v1.0)** | Открытый стандарт для inter-agent коммуникации. `a2a-sdk` для Python — async-first, поддержка JSON-RPC/HTTP/gRPC, extras для PostgreSQL и OpenTelemetry. |
| **Langfuse для трейсинга** | Трейсинг LLM-вызовов и агентских цепочек. Self-hosted, интегрируется с OpenTelemetry. |
...
| **PostgreSQL для состояния** | Хранение провайдеров, Agent Cards, конфигурации авторизации. Единая БД для упрощения операций. |
...
**Технологии:** FastAPI, httpx (async streaming), Pydantic v2
...
**Технологии:** FastAPI, a2a-sdk, asyncpg/SQLAlchemy
...
- Запуск Codex CLI в режиме `exec` или через MCP server
- Codex выполняет read-only диагностику в терминале целевого хоста
- Поиск релевантных Runbooks в Qdrant
- Формирование структурированного отчёта → Telegram
- Регистрация как A2A-агент при старте

**Технологии:** Codex CLI, FastAPI (webhook receiver), MCP, Qdrant client, Telegram Bot API
...
**Компоненты:**
- Тестовое приложение (Python/FastAPI или Java + PostgreSQL + Redis)
...
9. [Codex] Получает ответ, решает выполнить bash-команду
        │  (docker stats, top, df -h, tail logs, etc.)
        │
10. [Codex → Terminal] Выполняет read-only команду в sandbox
         │
...
14. [Codex → MCP Tool: Telegram] Отправляет отчёт в Telegram
         │
15. [Langfuse] Весь trace записан: шаги, промпты, ответы LLM, tool calls
```
...
**Индексация:**
- При старте/обновлении: парсинг `.md` файлов → chunking → embedding (модель: `text-embedding-3-small` через Gateway или локальная) → upsert в Qdrant
...
|---|---|---|
| Shell execution | Bash-команды на целевом хосте | Sandbox: read-only, whitelist бинарников, network disabled |
| File read | Чтение файлов (конфиги, логи) | В пределах рабочей директории / mounted volumes |
...
- **Shell-команды** ограничены whitelist (cat, ls, grep, top, df, docker stats/logs, journalctl, tail, head, ps, free, netstat)
...
- LLM API keys зашифрованы в PostgreSQL (Fernet / AES)
- Agent tokens — JWT с expiration
- Codex sandbox: read-only, network disabled (кроме Gateway endpoint)
- Whitelist shell-команд на уровне AGENTS.md + Python wrapper
